...
import { AccountExpiry, AccountExpiryFormatter } from '../shared/account-expiry';
...
class ApplicationMain {
  ...
  private setTunnelState(newState: TunnelState) {
    ...
    const accountExpiry = this.accountData ? new AccountExpiry(this.accountData.expiry) : undefined;
  ...
  private async autoConnect() {
    if (!this.accountData || !new AccountExpiry(this.accountData.expiry).hasExpired()) {
      try {
  ...
  private handleAccountExpiry() {
    if (this.accountData) {
      const accountExpiry = new AccountExpiryFormatter(this.accountData.expiry, this.locale);
      ...
      if (accountExpiry.hasExpired()) {
        ...
        if (closeToExpiryNotification.mayDisplay()) {
          this.notificationController.notify(closeToExpiryNotification.getSystemNotification());

          const twelveHours = 12 * 60 * 60 * 1000;
          const delay = Math.min(twelveHours, accountExpiry.remainingMilliseconds());
